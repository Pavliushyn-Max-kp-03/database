...
namespace ShopGUI
{
        ...
        private void Add()
        {
            ...
            if (long.TryParse(productIdInput.Text.ToString(), out n))
            {
                ...
                if (p.id != 0)
                {
                    ...
                    {
                        MessageBox.Query("Error", "This item is not available. Choose another one!", "OK");
                    }
    ...
    public class EditProduct : Dialog
    {
        ...
        public EditProduct(ProductRepository productRep, long id)
        {
            this.id = id;
    ...
    public class EditOrder : Dialog
    {
        ...
        public EditOrder(OrderRepository orderRep, ProductRepository productRep, long id)
        {
    ...
    public class ExportDialog : Dialog
    {
        ...
        public void OnCreateDialogSubmit()
        {
            ImportAndExport.Export(Convert.ToInt64(orderIdInput.Text.ToString()), exportPathInput.Text.ToString(), OrderRep, ProductRep);
            Application.RequestStop();
        }
        public void SetRepository(UserRepository UserRep, OrderRepository OrderRep, ProductRepository ProductRep)
        {
    ...
    public class ImportDialog : Dialog
    {
        ...
        {
            bool error;
            ImportAndExport.Import(Convert.ToInt64(productIdInput.Text.ToString()), fullNameInput.Text.ToString(), OrderRep,out error);
            if (error)
                MessageBox.ErrorQuery("Error", "Import failed. Invalid file extension.", "OK");
            else
    ...
    public class SearchProduct: Dialog
    {
        ...
        public void OnCreateDialogSubmit()
        {
            ...
            if(list.Count == 0)
            {
                MessageBox.Query("Error", $"There aren`t products by such name!", "OK");
            }
            else
            {
